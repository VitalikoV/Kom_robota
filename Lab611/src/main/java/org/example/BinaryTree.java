...
public class BinaryTree {
    ...
    public File get(String val, Date createdDate){
        return get(root, val, createdDate);
    }

    private File get(CustomNode node, String val, Date createdDate){
        if(node == null){
            return null;
        }

        if(Objects.equals(val, node.getValue().getName())){
            return node.getValue();
        }

        return createdDate.before(node.getValue().getCreatedDate())? get(node.getLeft(), val, createdDate) : get(node.getRight(), val, createdDate);
    ...
    private Boolean containsValue(CustomNode node, File val){
        ...
        return val.getCreatedDate().before(node.getValue().getCreatedDate())? containsValue(node.getLeft(), val) : containsValue(node.getRight(), val);
